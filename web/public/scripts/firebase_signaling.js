...
function FirebaseSignalingChannel(deviceId, remoteVideo) {
	...
	if (this.checkUserSignedIn() == false) {
		return new Error(
			'User need to sign in before using FirebaseSignalingChannel'
		);
	}
}
...
FirebaseSignalingChannel.prototype.initSignaling_ = function() {
	...
	this.devicePresenceRef_ = this.database_.ref(
		'devices/' + this.auth_.currentUser.uid + '/' + this.deviceId_
	);
	...
	this.messagesRef_ = this.database_.ref(
		'messages/' + this.auth_.currentUser.uid + '/' + this.deviceId_
	);
	...
	this.keepalive_interval = window.setInterval(
		this.sendKeepAliveMessage.bind(this),
		1000
	);
};
...
FirebaseSignalingChannel.prototype.onRecvDatabaseMessage_ = function(snap) {
	var val = snap.val();
	var messageTimeDiff =
		new Date().getTime() + this.serverTimeOffset_ - val.timestamp;
	trace(
		'Message : To: ' +
			val.to +
			', deviceid: ' +
	...
	if (val.to == 'server') {
		if (messageTimeDiff < this.signalingMessageTimeout_) {
			...
			if (dataJson['cmd'] == 'send') {
				this.peerConnectionClient_.onReceivePeerMessage(
					dataJson['msg']
				);
			} else {
...
FirebaseSignalingChannel.prototype.doSignalingConnect = function() {
	...
	this.peerConnectionClient_ = new PeerConnectionClient(
		this.remoteVideo_,
		this.doSignalingSend.bind(this)
	);
};
...
// open the door
FirebaseSignalingChannel.prototype.doSignalingDoor = function() {
	// Sending open door meessage to client
	var door_message = {
		cmd: 'door',
		msg: JSON.stringify({ type: 'open' }),
		error: ''
	};
	this.sendDatabaseMessage_(JSON.stringify(door_message));
};

FirebaseSignalingChannel.prototype.doSignalingSend = function(data) {
	var message = { cmd: 'send', msg: JSON.stringify(data), error: '' };
