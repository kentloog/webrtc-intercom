...
var kConstant_DeviceTableTemplate =
	...
	'<td>' +
	'<button class="mdl-button mdl-js-button mdl-button--raised" onclick=ShowVideoStream("TMPL_DEVICEID")>Connect</button>' +
	'</td>';
...
DeviceTable.prototype.updateDeviceTable = function() {
	...
	var query = this.database_
		.ref('devices/' + this.auth_.currentUser.uid)
		.orderByKey();
	trace('Device list ref : ' + query);
	trace('Start Query ---------------');
	query.once('value').then(this.updateDeviceTableDom_.bind(this));
	trace('End Query ---------------');
};
...
DeviceTable.prototype.updateDeviceTableDom_ = function(snapshot) {
	...
	if (!table_record) {
		...
		table_record.innerHTML = table_record.innerHTML
			.replace(/TMPL_TITLE/g, val.title)
		...
		table_record.innerHTML = messageElement.innerHTML.replace(
			/TMPL_DEVICEID/g,
			val.deviceid
		);
		table_record.querySelector('.title').textContent = val.title;
		table_record.querySelector('.description').textContent =
			val.description;
