...
function PeerConnectionClient(remoteVideo, doSendPeerMessage) {
	...
	this.peerConnection_.onremovestream = trace.bind(
		null,
		'Remote stream removed.'
	);
	this.peerConnection_.onsignalingstatechange = this.onSignalingStateChanged_.bind(
		this
	);
	this.peerConnection_.oniceconnectionstatechange = this.onIceConnectionStateChanged_.bind(
		this
	);
	trace(
		'Created RTCPeerConnnection with config: ' +
			JSON.stringify(this.pcConfig_)
	);
}
...
PeerConnectionClient.prototype.onIceConnectionStateChanged_ = function() {
	...
	trace(
		'ICE connection state changed to: ' +
			this.peerConnection_.iceConnectionState
	);
...
PeerConnectionClient.prototype.getPeerConnectionStats = function(callback) {
	...
	this.peerConnection_.getStats(null).then(callback);
};
...
PeerConnectionClient.prototype.close = function() {
	...
	window.dispatchEvent(
		new CustomEvent('peerconnectionclosed', {
			detail: {
				pc: this,
				time: new Date()
			}
...
PeerConnectionClient.prototype.setLocalSdpAndSend_ = function(
	sessionDescription
) {
