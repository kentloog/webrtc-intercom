...
const smsButton = require('./lib/send_sms.js');
...
commander
	...
	.option(
		'--config-file [config]',
		'specify the path of the config file in the option'
	)
	.parse(process.argv);
...
if (!fs.existsSync(rws_config_filenam)) {
	logger.error('Configuration file not found! : ' + rws_config_filenam);
	process.exit(-1);
}
...
if (config_.auth_type === 'email-password') {
	...
	let local_params = Object.assign(
		proxy_config,
		auth_params_,
		device_params_
	);
	...
	logger.error(
		'Not supported auth type in proxy_config.json :' + config_.auth_type
	);
}
