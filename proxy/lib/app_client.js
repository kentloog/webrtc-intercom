...
const door = require('./open_door.js');
...
module.exports = class AppClient {
	constructor(url, sendMessage, onAppClientObserver, deviceId) {
		...
		this.websocketClient_ = new WebSocketClient(
			this.url_,
			this.doSendToServer.bind(this),
			this.onWebSocketObserver.bind(this)
		);
	}
	...
	activateSession_(roomid, clientid) {
		...
		this.keepalive_interval_ = setInterval(
			this.onKeepAliveInterval.bind(this),
			1000
		);
	...
	validateSession_(roomid, clientid) {
		...
		if (this.session_state_ == SESSION_CONNECTED) {
			...
			if (this.roomId_ == roomid && this.clientId_ == clientid) {
				...
				logger.error(
					'Invalid Session Client id(Session): R:' +
						this.roomId_ +
						', C:' +
						this.clientId_
				);
				logger.error(
					'Invalid Session Client id(Message): R:' +
						roomid +
						', C:' +
						clientid
				);
			...
			logger.error(
				'Invalid session state during validating session: ' +
					this.session_state_
			);
		}
		return false;
	...
	onKeepAliveInterval() {
		...
		if (update_difference > 4000 && update_difference < 7000) {
			logger.error(
				'Sending bye command triggered by KeepAlive timeout : ' +
					update_difference
			);

			// send bye message to device
			this.websocketClient_.doSendMessage(
				JSON.stringify({
					cmd: 'send',
	...
	doSendToDevice(app_message) {
		...
		switch (json_message.cmd) {
			case MSG_CMD_REGISTER:
				if (
					this.validateSession_(
						json_message.roomid,
						json_message.clientid
					) == true
				) {
					// sending message
					this.activateSession_(
						json_message.roomid,
						json_message.clientid
					);
					if (this.websocketClient_.isConnected()) {
						logger.info(
							'Server -> Device : ' +
								JSON.stringify(app_message.message)
						);
						this.websocketClient_.doSendMessage(
							app_message.message
						);
						...
						logger.error(
							'WebSocketClient is not ready to send register command'
						);
					}
				} else {
			...
			case MSG_CMD_SEND:
				...
				if (
					...
					if (this.websocketClient_.isConnected()) {
						...
						logger.info(
							'WebSocketClient is not ready to send(send)'
						);
					}
				} else {
					// print validation result
	...
	getHardwareDeviceId_() {
		...
		if (serial.length < 2) return '000000005b8879cc';
