...
module.exports = class FirebaseClientGoogleApi {
	constructor(config) {
		if (config == undefined) {
			logger.error(
				'No Configuration parameters to initialize Firebase Client'
			);
		}
		logger.debug(
			'Firebase Config file path: ' + config.client_firebase_config_path
		);
		let firebase_config = JSON.parse(
	...
	initFirebaseClient() {
		this.app_client_ = new AppClient(
			'ws://10.0.0.11:8889/rws/ws',
		...
		this.messagesRef_ = this.database_.ref(
			'messages/' + this.uid_ + '/' + this.deviceId_
		);
	...
	initDeviceInfo() {
		...
		this.presenceRef_ = this.database_.ref(
			'devices/' + this.uid_ + '/' + this.deviceId_
		);
	...
	updateDeviceInfoSession(conn_status) {
		...
		switch (conn_status) {
			...
			default:
				logger.error(
					'Unknown AppClient Connection status: ' + conn_status
				);
				return;
		}
